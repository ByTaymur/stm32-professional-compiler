...
async function buildProject() {
    ...
    if (!ws)
        return;
    const config = vscode.workspace.getConfiguration('stm32-pro');
    const profile = config.get('defaultProfile') || 'O2';
    ...
    try {
        ...
        const result = (0, child_process_1.execSync)(buildCmd, {
            cwd: ws,
            encoding: 'utf8',
...
async function flashFirmware() {
    ...
    try {
        ...
        if (fs.existsSync(buildDir)) {
            ...
            if (elf)
                elfFile = path.join(buildDir, elf);
        }
        if (!elfFile) {
        ...
        const config = vscode.workspace.getConfiguration('stm32-pro');
        const openocdPath = config.get('openocdPath') || 'openocd';
        ...
        const result = (0, child_process_1.execSync)(flashCmd, { cwd: ws, encoding: 'utf8' });
        outputChannel.appendLine(result);
...
async function detectProgrammer() {
    try {
        (0, child_process_1.execSync)('lsusb | grep -i "st-link"', { encoding: 'utf8' });
        return 'stlink-v2';
    }
    catch { }
    try {
        (0, child_process_1.execSync)('lsusb | grep -i "j-link"', { encoding: 'utf8' });
        return 'jlink';
    }
    catch { }
    try {
        (0, child_process_1.execSync)('lsusb | grep -i "cmsis-dap"', { encoding: 'utf8' });
        return 'cmsis-dap';
    }
    catch { }
...
async function cleanBuild() {
    ...
    outputChannel.appendLine('\n=== Cleaning Build ===');
    try {
        (0, child_process_1.execSync)('make clean', { cwd: ws, encoding: 'utf8' });
...
async function testConnection() {
    ...
    try {
        const config = vscode.workspace.getConfiguration('stm32-pro');
        const openocdPath = config.get('openocdPath') || 'openocd';
        const result = (0, child_process_1.execSync)(`${openocdPath} -f interface/stlink-v2.cfg -f target/stm32f7x.cfg -c "init; reset halt; exit"`, { cwd: ws, encoding: 'utf8', timeout: 10000 });
        outputChannel.appendLine(result);
...
function disconnect() {
    ...
    try {
        (0, child_process_1.execSync)('pkill -9 openocd', { stdio: 'ignore' });
    }
    catch { }
    statusBarItem.text = `$(circuit-board) STM32`;
...
async function setupDebugConfig(ws) {
    const vscodePath = path.join(ws, '.vscode');
    if (!fs.existsSync(vscodePath)) {
        fs.mkdirSync(vscodePath, { recursive: true });
    }
...
function autoDetectProject(context) {
    const ws = getWorkspace();
    if (!ws)
        return;
